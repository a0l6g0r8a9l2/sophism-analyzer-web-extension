...
- Structured output: `responseMimeType: "application/json"` + `responseSchema` (built by `buildResponseSchema()`) force valid JSON matching the `Fallacy` shape. `type`, `category`, `severity` are closed enums in the schema — expressed via the `enum` array **only** (do NOT add `format: "enum"`; Gemini's `Schema.format` accepts only numeric subtypes like `int32`/`float` and rejects other values with `400 INVALID_ARGUMENT`). The parser (`parseAnalysisResponse`) still keeps a regex-extraction fallback for non-JSON-wrapped payloads and validates enum values defensively.
...
- Transcript: `analyzeVideo` requests the transcript from the originating tab via `GET_TRANSCRIPT`, renders it via `buildTranscriptText` (with a char budget), and injects it as a text part so the model anchors `timestamp` to real segment starts. `fileData.fileUri` is set to a clean `https://www.youtube.com/watch?v=VIDEO_ID` URL **without** an explicit `mimeType` — Gemini infers the type from the URL, and an explicit mimeType on a URL-based fileUri is rejected with `400 INVALID_ARGUMENT`. When no transcript is available, the call degrades to video-only.
