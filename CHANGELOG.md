...
- Retry logic raced a new Gemini request against the previous, still in-flight `generateContent` promise: when the per-attempt timeout fired, the abandoned HTTP call kept running, so retries could overlap and double-spend API quota. The retry loop now waits for the previous API call to actually settle (capped at 2× timeout for the first retry and 3× for the second, to avoid stalling on a permanently hung SDK) before applying `apiRetryDelay`, and clears both the main and settle timers via `clearTimeout` so the MV3 service worker can return to idle. ([#5](https://github.com/a0l6g0r8a9l2/sophism-analyzer-web-extension/issues/5))
